...
def activation_layer(activation: str="relu", alpha: float=0.1, inplace: bool=True):
    ...
    You give it an activation function name (for now leaky_relu or relu) and it returns the corresponding activation layer.
    """
    if activation == "relu":
...
class Network(nn.Module):
    """ Complete OCR for handwriting neural network"""
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        # normalize between 0 and 1 the images
        images_flaot = images / 255.0

        # transpose image to channel
