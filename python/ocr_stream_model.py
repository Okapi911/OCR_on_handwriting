...
# Instatiate model
model = ImageToWordModel(model_path="models/202402061644/model.onnx") #Change the path to test another model
...
try:
    while True:
        ...
        result = model.predict(color_image) #Repeat this syntax to test images that weren't in the original dataset 
    
        publish_prediction(client, result)
        plt.pause(0.5)
        # 
        # for res in results:
